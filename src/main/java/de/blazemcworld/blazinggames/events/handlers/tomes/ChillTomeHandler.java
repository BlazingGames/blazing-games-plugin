...
public class ChillTomeHandler extends BlazingEventHandler<LootGenerateEvent> {
    @Override
    public boolean fitCriteria(LootGenerateEvent event) {
        NamespacedKey key = event.getLootTable().getKey();
        return key.equals(LootTables.IGLOO_CHEST.getKey());
    }
    ...
    public void execute(LootGenerateEvent event) {
        ...
        if (getBook <= 40) {
            event.getLoot().add(CustomItems.CHILL_TOME.create());
        }
    }
}
